...
def run_datasets_in_parallel(file, chembl_info):
    ...
    if y.nunique() == 1 or get_repetition_rate(np.array([y]).T) >= 0.85:
        ...
        invalid_datasets = np.load(os.getcwd() + "/extrapolation_xgb_reg_chembl/" + "invalid_datasets.npy")
        invalid_datasets.append(data_id)
        np.save(os.getcwd() + "/extrapolation_xgb_reg_chembl/" + "invalid_datasets.npy", invalid_datasets)
        return
    ...
    np.save(os.getcwd() + "/extrapolation_xgb_reg_chembl/" + "extrapolation_xgb_reg_chembl_cv_"+str(data_id)+".npy", metrics)
    return
...
def count_finished_datasets(sorted_chembl_info):
    ...
    for file in range(len(sorted_chembl_info)):
        ...
        if data_id in invalid_datasets:
            ...
            continue
        try:
            _ = np.load(os.getcwd() + "/extrapolation_xgb_reg_chembl/" + "extrapolation_xgb_reg_chembl_cv_"+str(data_id)+".npy",)
            existing_count += 1
...
if __name__ == '__main__':
    ...
    try:
        _ = np.load(os.getcwd() + "/extrapolation_xgb_reg_chembl/" + "invalid_datasets.npy")
    except:
        np.save(os.getcwd() + "/extrapolation_xgb_reg_chembl/" + "invalid_datasets.npy", [])
